...
//					pp.panel('resize');
	...
	function collapsePanel(container, region, animateSpeed){
		...
		function getOption(){
			...
			if (region == 'east'){
				var ww = copts.width + popts.width - csize;
				...
				if (!isVisible(panels.expandNorth)){
					hh += popts.height - csize + ((popts.split || !popts.border)?1:0);
				}
...
(function($){
	...
	function setSize(target, width){
		...
		bleft.find('span.l-btn-text').css({
	...
	function bindEvents(target){
		...
		if (!opts.disabled){
			input.bind('blur.searchbox', function(e){
				opts.value = $(this).val();
				if (opts.value == ''){
			...
			button.bind('click.searchbox', function(){
				opts.searcher.call(target, opts.value, input._propAttr('name'));
	...
	function setDisabled(target, disabled){
		...
		var opts = state.options;
		var input = state.searchbox.find('input.searchbox-text');
		var mb = state.searchbox.find('a.searchbox-menu');
		if (disabled){
			opts.disabled = true;
			$(target).attr('disabled', true);
			input.attr('disabled', true);
			if (mb.length){mb.menubutton('disable');}
		} else {
			opts.disabled = false;
			$(target).removeAttr('disabled');
			input.removeAttr('disabled');
			if (mb.length){mb.menubutton('enable');}
		}
	}
...
(function($){
	 ...
	 */
	function init(target){
		$(target).addClass('numberbox numberbox-f');
	...
	function setSize(target, width){
		...
		var spacer = $('<div style="display:none"></div>').insertBefore(t);
		t.appendTo('body');
...
(function($){
	function create(target){
		...
		$(target).spinner(opts).numberbox($.extend({},opts,{width:'auto'}));
	}
...
//		},
	...
	$.fn.combo.parseOptions = function(target){
		var t = $(target);
		return $.extend({}, $.fn.validatebox.parseOptions(target), $.parser.parseOptions(target, [
			'width','height','separator','panelAlign',
